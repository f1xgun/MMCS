...
--*/
{
    ...
    if (NT_SUCCESS(status)) {
        if (RtlEqualUnicodeString(&required_extension, &NameInfo->Extension, FALSE) == TRUE) {
            DbgPrint("[PassThrough] NameInfo->Extension = %wZ\n", NameInfo->Extension);

            if (Data->Iopb->MajorFunction == IRP_MJ_WRITE) {
                DbgPrint("[PassThrough] Writing to file: %wZ | %wZ\n", NameInfo->Name, NameInfo->Extension);
                if (Data->Iopb->Parameters.Write.WriteBuffer) {
                    DbgPrint("[PassThrough] InBuffer: %s\n", (char*)Data->Iopb->Parameters.Write.WriteBuffer);
                    uint8_t key[32] = { 0x74, 0xdf, 0xeb, 0x8e, 0xc2, 0x84, 0x61, 0xdf, 0x05, 0xc5, 0x47, 0x89, 0x65, 0x1d, 0x7c, 0x40,
                                    0x58, 0x3e, 0xe1, 0x62, 0x72, 0x9a, 0x3d, 0x93, 0x5f, 0x7f, 0xbd, 0x70, 0x76, 0xae, 0x69, 0x71 };
                    uint8_t iv[16] = { 0xf8, 0xae, 0xde, 0xdb, 0x1d, 0x6a, 0x2f, 0xff, 0x85, 0xa8, 0x77, 0x5c, 0x05, 0xd1, 0xa3, 0xa4 };
                    ULONG bufferSize = Data->Iopb->Parameters.Write.Length;
                    uint8_t* buffer = (uint8_t*)ExAllocatePoolWithTag(NonPagedPool, bufferSize, 'BUFF');
                    if (buffer == NULL) {
                        DbgPrint("[PassThrough] Failed to allocate memory for buffer\n");
                        return;
                    }
                    memcpy(buffer, Data->Iopb->Parameters.Write.WriteBuffer, bufferSize);

                    struct AES_ctx ctx;
                    AES_init_ctx_iv(&ctx, key, iv);
                    AES_CBC_encrypt_buffer(&ctx, buffer, bufferSize);
                    memcpy(Data->Iopb->Parameters.Write.WriteBuffer, buffer, bufferSize);
                    ...
                    // Если буфер записи оказался пустым, сообщаем об этом
    ...
    if (NT_SUCCESS(status)) {
        if (RtlEqualUnicodeString(&required_extension, &NameInfo->Extension, FALSE) == TRUE) {
            ...
            if (Data->Iopb->MajorFunction == IRP_MJ_READ) {
                DbgPrint("[PassThrough] Reading from file: %wZ | %wZ\n", NameInfo->Name, NameInfo->Extension);
                if (Data->Iopb->Parameters.Read.ReadBuffer) {
                    DbgPrint("[PassThrough] InBuffer: %s\n", (char*)Data->Iopb->Parameters.Read.ReadBuffer);
                    uint8_t key[32] = { 0x74, 0xdf, 0xeb, 0x8e, 0xc2, 0x84, 0x61, 0xdf, 0x05, 0xc5, 0x47, 0x89, 0x65, 0x1d, 0x7c, 0x40,
                                    0x58, 0x3e, 0xe1, 0x62, 0x72, 0x9a, 0x3d, 0x93, 0x5f, 0x7f, 0xbd, 0x70, 0x76, 0xae, 0x69, 0x71 };
                    uint8_t iv[16] = { 0xf8, 0xae, 0xde, 0xdb, 0x1d, 0x6a, 0x2f, 0xff, 0x85, 0xa8, 0x77, 0x5c, 0x05, 0xd1, 0xa3, 0xa4 };
                    ULONG bufferSize = Data->Iopb->Parameters.Read.Length;
                    uint8_t* buffer = (uint8_t*)ExAllocatePoolWithTag(NonPagedPool, bufferSize, 'BUFF');
                    if (buffer == NULL) {
                        DbgPrint("[PassThrough] Failed to allocate memory for buffer\n");
                        return;
                    }
                    memcpy(buffer, Data->Iopb->Parameters.Read.ReadBuffer, bufferSize);
                    struct AES_ctx ctx;
                    AES_init_ctx_iv(&ctx, key, iv);
                    AES_CBC_decrypt_buffer(&ctx, buffer, bufferSize);
                    memcpy(Data->Iopb->Parameters.Read.ReadBuffer, buffer, bufferSize);
                    DbgPrint("[PassThrough] OutBuffer: %s Length: %lu\n", (char*)Data->Iopb->Parameters.Read.ReadBuffer, bufferSize);
